...
module.exports = {
  ...
  // [KULLANICI ŞİKAYETİ, KARARLAŞTIRILDI — ÇARK "PITY" SİSTEMİ] "Bazen hep aynı şey geliyor,
  // 5 tur kötü gelirse sonra iyiye döner mi yoksa şans hep aynı mı?" sorusuna verilen cevap:
  // önceden HAYIR'dı — spinWheelSegment() geçmişten tamamen habersiz, saf ağırlıklı rastgele
  // seçimdi (klasik IID), art arda kötü gelme ihtimali istatistiksel olarak tamamen mümkündü ve
  // hiç telafi edilmiyordu. Kullanıcıya 3 seçenek sunuldu (pity ekle / sadece ağırlıkları
  // dengele / ikisi birden / hiçbiri), kullanıcı "ne önerirsin" dedi, önerilen "pity sistemi"
  // seçildi. Tasarım: her katılımcı için KENDİ turlarında ayrı bir "art arda kaç kez kötü
  // havuzdan segment geldi" sayacı tutulur (room.draft.wheelPity[clientId], bkz.
  // DraftEngine.resolveSpin) — kötü havuzdan gelince +1, iyi havuzdan gelince sıfırlanır, orta
  // havuz sayaç için nötr (ne artırır ne sıfırlar). Bir sonraki çevirmede bu sayaç kadar, o
  // draftın çarkındaki 'iyi' havuz segmentlerinin ağırlığı katlanarak artırılır (diğer
  // segmentlerin ağırlığı DEĞİŞMEZ) — çark hâlâ tamamen şansa dayalı/deterministik DEĞİL (aynı
  // sayaçla bile kötü gelebilirsiniz), ama uzun kötü serilerin olasılığı sayaç arttıkça hızla
  // düşer. MAX_STREAK'te tavanlanır ki etkisi sınırsız büyümesin. Bkz. pool.js
  // spinWheelSegment(segments, pityBoost).
  WHEEL_PITY_BOOST_PER_STREAK: 0.6,
  WHEEL_PITY_MAX_STREAK: 5,

  // [KULLANICI İSTEĞİ, KARARLAŞTIRILDI — HAZIRLIK ÇARKI] "Kör draft ve açık arttırma için
  // geçerli, oyun başlamadan önce (formasyon kurasından ÖNCE, kullanıcı onayladı) bütün
  // kullanıcılar isteğe bağlı bir çark çevirebilir — iyi/kötü karışık perk'ler. Bu eklenti
  // isteğe bağlı olsun (host oda kurarken açar/kapatır), çark seçiliyken bile HER oyuncunun
  // kendi çevirmesi isteğe bağlı kalsın (risk var, hiç risk almadan da başlanabilmeli)."
  // Çark Modu'nda (draftMode==='wheel') hiç gösterilmez/uygulanmaz — RoomManager.createRoom bunu
  // zorluyor (bkz. orada).
  // [KULLANICI İSTEĞİ, KARARLAŞTIRILDI — "TAM SÜRÜM"] "Açıklama getir çarka, çarktaki şeylerin
  // özelliklerini açıkla, oyuncular bilsin." — her segmente `description` eklendi, client bunu
  // hem draft başlamadan (tüm katalog bir legend olarak) hem her sonuçta gösteriyor (bkz.
  // views.js renderPrepWheel). Ayrıca MVP'de "cascade internals'ına derin dokunuş" riski
  // yüzünden ertelenen iki fikir ("Joker Turu", "Gözcü") artık burada — bkz. DraftEngine.js
  // nextRound (joker) / peekBids (spy).
  PREP_WHEEL_SEGMENTS: [
    ...
    { kind: 'budget_penalty', label: '💸 Bütçe Cezası', description: 'Bütçenden anında -150₺ düşülür.', pool: 'kötü', weight: 10 },
    { kind: 'ceiling_reduction', label: '📉 Tavan Düşüşü', description: 'Draft boyunca sürekli aktif: kişisel maksimum teklif tavanın %20 daha düşük hesaplanır.', pool: 'kötü', weight: 8 },
    { kind: 'blind_first_round', label: '🙈 Kör İlk Tur', description: 'Katıldığın İLK turda, ana oyuncunun reytingini KENDİ EKRANINDA (bulanıklaştırılmış) göremezsin — kendine seçtiğin bir zorluk, rakibe karşı bir sır değil.', pool: 'kötü', weight: 8 },
  ],
  // [KULLANICI İSTEĞİ, KARARLAŞTIRILDI — "TAM SÜRÜM"] "Çarkı sıra sıra çevirsinler, ilk başta
  // 1. oyuncu, ona çıkan şeyi herkes görsün, sonra diğer oyuncuya geçsin." — hazırlık çarkı
  // artık N kişilik odada eşzamanlı/bağımsız DEĞİL, ÇARK MODU'ndaki gibi TURN-BASED (bkz.
  // DraftEngine.openPrepWheelTurn) — bu süre artık FAZIN TAMAMI değil, AKTİF oyuncunun TEK
  // turu için.
  PREP_WHEEL_PICK_DURATION_SECONDS: Number(process.env.DRAFT_PREP_WHEEL_SECONDS) || 20,
  PREP_WHEEL_BUDGET_BONUS: 150,
