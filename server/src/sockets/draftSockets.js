...
function registerDraftSockets(io, socket, ctx) {
  ...
  socket.on('draft:start', ({ code } = {}, cb) => {
    ...
    // RoomManager.createRoom), draft doğrudan başlamaz — önce isteğe bağlı hazırlık çarkı fazı
    // (formasyon kurasından ÖNCE) araya girer; asıl startDraft (kura dahil) bu faz bitince
    // otomatik çağrılır (bkz. DraftEngine.checkPrepWheelComplete/autoResolvePrepWheel).
    const result = room.prepWheelEnabled ? draftEngine.startPrepWheelPhase(room) : draftEngine.startDraft(room);
    cb?.(result);
  });

  // [KULLANICI İSTEĞİ, KARARLAŞTIRILDI — HAZIRLIK ÇARKI] Her oyuncu kendi kararını verir —
  // çevir (risk var) ya da atla (hiç risk almadan devam et).
  socket.on('draft:prepWheelSpin', ({ code } = {}, cb) => {
    const room = getRoom(code);
    if (!room) return cb?.({ error: 'ROOM_NOT_FOUND' });
    cb?.(draftEngine.spinPrepWheel(room, socket.data.clientId));
  });
  socket.on('draft:prepWheelSkip', ({ code } = {}, cb) => {
    const room = getRoom(code);
    if (!room) return cb?.({ error: 'ROOM_NOT_FOUND' });
    cb?.(draftEngine.skipPrepWheel(room, socket.data.clientId));
  });

  // [KULLANICI İSTEĞİ, KARARLAŞTIRILDI — HAZIRLIK ÇARKI "TAM SÜRÜM"] "👁️ Gözcü" perk'i — SADECE
  // bu çağıran istemciye (ACK, broadcast DEĞİL) kör draftın o anki round.bids'ini bir kez açar.
  socket.on('draft:peekBids', ({ code } = {}, cb) => {
    const room = getRoom(code);
    if (!room) return cb?.({ error: 'ROOM_NOT_FOUND' });
    cb?.(draftEngine.peekBids(room, socket.data.clientId));
  });
