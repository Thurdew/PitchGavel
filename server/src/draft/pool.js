 ...
 *
 * [KULLANICI ŞİKAYETİ, KARARLAŞTIRILDI — PITY SİSTEMİ] `pityBoost` (varsayılan 1 = etkisiz):
 * çağıran taraf (bkz. DraftEngine.resolveSpin) bunu, o katılımcının art arda kaç kez 'kötü'
 * havuzdan segment aldığına göre hesaplayıp geçirir — 1'den büyükse SADECE pool==='iyi'
 * segmentlerin ağırlığı bu katsayıyla çarpılır, diğer segmentler DEĞİŞMEDEN kalır. Böylece uzun
 * kötü seriler istatistiksel olarak hızla düşürülür ama sonuç asla garanti/deterministik olmaz. */
function spinWheelSegment(segments, pityBoost = 1) {
  const weightFor = (s) => (pityBoost > 1 && s.pool === 'iyi' ? s.weight * pityBoost : s.weight);
  const total = segments.reduce((sum, s) => sum + weightFor(s), 0);
