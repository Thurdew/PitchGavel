...
function registerRoomSockets(io, socket, ctx) {
  ...
  socket.on('room:create', ({ clientId, name, draftMode, playerPool, wheelSegmentLabels, prepWheelEnabled } = {}, cb) => {
    if (!clientId) return cb?.({ error: 'CLIENT_ID_REQUIRED' });
    const room = roomManager.createRoom(clientId, name, draftMode, playerPool, wheelSegmentLabels, prepWheelEnabled);
