...
const {
  ...
  WHEEL_RATING_BANDS,
  WHEEL_PITY_BOOST_PER_STREAK,
  WHEEL_PITY_MAX_STREAK,
  PREP_WHEEL_SEGMENTS,
  PREP_WHEEL_PICK_DURATION_SECONDS,
  PREP_WHEEL_BUDGET_BONUS,
  PREP_WHEEL_BUDGET_PENALTY,
  PREP_WHEEL_GAMBLER_AMOUNT,
  PREP_WHEEL_CEILING_REDUCTION,
...
// [KULLANICI İSTEĞİ, KARARLAŞTIRILDI — HAZIRLIK ÇARKI] "📉 Tavan Düşüşü" perk'i — draft boyunca
// SÜREKLİ aktif (tek kullanımlık değil), kişisel tavanı PREP_WHEEL_CEILING_REDUCTION kadar kısar.
function personalMaxBid(player) {
  const remainingSlotsTotal = SQUAD_SIZE - player.squad.length;
  if (remainingSlotsTotal <= 0) return 0;
  let cap = player.budget - (remainingSlotsTotal - 1) * MIN_PLAYER_PRICE;
  if (player.prepPerk && player.prepPerk.kind === 'ceiling_reduction' && player.prepPerk.active) {
    cap = Math.floor(cap * (1 - PREP_WHEEL_CEILING_REDUCTION));
  }
  return cap;
}
...
class DraftEngine {
  ...
  // başlamadan önce (formasyon kurasından ÖNCE) bütün kullanıcılar isteğe bağlı bir çark
  // çevirecek — iyi/kötü perk'ler. Eklenti isteğe bağlı olsun (host lobide karar verir), çark
  // seçiliyken bile her oyuncunun çevirmesi isteğe bağlı kalsın — risk var, hiç risk almadan da
  // başlanabilmeli." — draftSockets.js `draft:start`, room.prepWheelEnabled ise bu fazı,
  // değilse doğrudan startDraft'ı çağırır. Faz bitince startDraft(room) NORMAL akışıyla
  // (formasyon kurası dahil) devam eder.
  //
  // [KULLANICI İSTEĞİ, KARARLAŞTIRILDI — "TAM SÜRÜM"] "Çarkı sıra sıra çevirsinler — ilk başta
  // 1. oyuncu, ona çıkan şeyi herkes görsün, sonra diğer oyuncuya geçsin." — ilk sürümde N
  // kişilik odada herkes AYNI ANDA/BAĞIMSIZ karar veriyordu; bu tamamen terk edildi. Artık Çark
  // Modu'ndaki (`nextWheelTurn`) ile AYNI desen: `room.prepWheel.order` (draft başında bir kez
  // karıştırılmış sabit sıra) + `cursor` ile TEK KİŞİLİK turlar halinde ilerler — sadece
  // `order[cursor]` çevirebilir/atlayabilir, diğerleri o turun sonucunu (herkese açık) izler.
  startPrepWheelPhase(room) {
    ...
    room.status = STATUS.PREP_WHEEL;
    room.prepWheel = {
      order: shuffleInPlace(room.players.map((p) => p.clientId)), // draft başında bir kez karışır, sabit kalır
      cursor: 0, // order[cursor] şu an sırası gelen oyuncu
      deadline: null,
      timer: null,
    };
    this.openPrepWheelTurn(room);
    return { ok: true };
  }

  // Sıradaki oyuncunun turunu açar (ya da herkes bittiyse fazı kapatıp asıl draftı başlatır).
  openPrepWheelTurn(room) {
    const pw = room.prepWheel;
    if (!pw) return;
    if (pw.cursor >= pw.order.length) {
      room.prepWheel = null;
      this.startDraft(room); // formasyon kurası dahil normal draft başlangıcı
      return;
    }
    pw.deadline = Date.now() + PREP_WHEEL_PICK_DURATION_SECONDS * 1000;
    // [SAVUNMA] Sırası gelen oyuncu hiç karar vermezse (bağlantı kopması vb.) faz asla
    // tıkanmasın — süre dolunca sunucu onun adına OTOMATİK "risk almadan devam et"i seçer
    // (asla otomatik ÇEVİRMEZ — zorla risk aldırmak istenmedi).
    clearTimeout(pw.timer);
    pw.timer = setTimeout(() => this.autoResolvePrepWheelTurn(room), PREP_WHEEL_PICK_DURATION_SECONDS * 1000);
    this.emitState(room);
  }

  advancePrepWheelTurn(room) {
    const pw = room.prepWheel;
    if (!pw) return;
    clearTimeout(pw.timer);
    pw.cursor += 1;
  ...
  applyPrepPerk(player, seg) {
    ...
    if (seg.kind === 'budget_bonus') {
      player.budget += PREP_WHEEL_BUDGET_BONUS;
      perk.detail = `+${PREP_WHEEL_BUDGET_BONUS}₺`;
      perk.active = false;
    } else if (seg.kind === 'budget_penalty') {
      player.budget = Math.max(0, player.budget - PREP_WHEEL_BUDGET_PENALTY);
      perk.detail = `-${PREP_WHEEL_BUDGET_PENALTY}₺`;
      perk.active = false;
    } else if (seg.kind === 'gambler') {
      const win = Math.random() < 0.5;
      const delta = win ? PREP_WHEEL_GAMBLER_AMOUNT : -PREP_WHEEL_GAMBLER_AMOUNT;
      player.budget = Math.max(0, player.budget + delta);
      perk.detail = win ? `+${PREP_WHEEL_GAMBLER_AMOUNT}₺ (şanslısın!)` : `-${PREP_WHEEL_GAMBLER_AMOUNT}₺ (şanssızlık!)`;
  ...
  spinPrepWheel(room, clientId) {
    ...
    // [KULLANICI İSTEĞİ, KARARLAŞTIRILDI — "TAM SÜRÜM"] "👁️ Gözcü" SADECE Kör Draft'ta anlamlı
    // — canlı modda tekliflerin zaten açık göründüğü için bu segment o odaların çarkında hiç
    // ÇIKMASIN diye spin anında havuzdan filtreleniyor (draft modundan bağımsız TEK bir katalog
    // kullanmaya devam ediyoruz, sadece bu draftın spin'i için aday listesi daralıyor).
    const available = room.draftMode === 'blind'
      ? PREP_WHEEL_SEGMENTS
      : PREP_WHEEL_SEGMENTS.filter((s) => s.kind !== 'spy');
    const seg = spinWheelSegment(available);
    const perk = this.applyPrepPerk(player, seg);
    this.io.to(room.code).emit('prepWheel:resolved', { clientId, perk });
    this.advancePrepWheelTurn(room);
    return { ok: true, perk };
  }

  // [KULLANICI İSTEĞİ, KARARLAŞTIRILDI] "Hiç risk almadan oyuna başlayabilir" — kendi isteğiyle
  // çevirmemeyi seçen bir oyuncu; hiçbir ceza/ödül almaz, sadece sırasını geçer.
  skipPrepWheel(room, clientId) {
    const pw = room.prepWheel;
    if (!pw || room.status !== STATUS.PREP_WHEEL) return { error: 'NO_ACTIVE_PREP_WHEEL' };
    if (pw.order[pw.cursor] !== clientId) return { error: 'NOT_YOUR_TURN' };
    if (!findPlayer(room, clientId)) return { error: 'NOT_IN_ROOM' };

    this.io.to(room.code).emit('prepWheel:resolved', { clientId, perk: null });
    this.advancePrepWheelTurn(room);
    return { ok: true, perk: null };
  }

  autoResolvePrepWheelTurn(room) {
    const pw = room.prepWheel;
    if (!pw || room.status !== STATUS.PREP_WHEEL) return;
    const clientId = pw.order[pw.cursor];
    this.io.to(room.code).emit('prepWheel:resolved', { clientId, perk: null, auto: true });
    this.advancePrepWheelTurn(room);
  }

  // [KULLANICI İSTEĞİ, KARARLAŞTIRILDI — "TAM SÜRÜM"] "👁️ Gözcü" — kör draftta, sırası gelen
  // oyuncu SADECE kendi ekranında (ACK, broadcast DEĞİL) o anki round.bids'i bir kez görebilir.
  // Bir kereye mahsus (kullanılınca active:false).
  peekBids(room, clientId) {
    const round = room.draft && room.draft.round;
    if (!round || round.kind !== 'blind_auction') return { error: 'NO_ACTIVE_BLIND_AUCTION' };
    if (!round.participantIds || !round.participantIds.includes(clientId)) return { error: 'NOT_A_PARTICIPANT' };
    const player = findPlayer(room, clientId);
    if (!player) return { error: 'NOT_IN_ROOM' };
    if (!player.prepPerk || player.prepPerk.kind !== 'spy' || !player.prepPerk.active) return { error: 'NO_SPY_AVAILABLE' };

    player.prepPerk.active = false;
    const bids = {};
    for (const id of round.participantIds) {
      const b = round.bids.get(id);
      bids[id] = b ? b.amount : null;
    }
    this.emitState(room); // prepPerk.active değişti, herkese açık görünürlük güncellensin
    return { ok: true, bids };
  }
  ...
  startDraft(room) {
    // [KULLANICI İSTEĞİ, KARARLAŞTIRILDI — HAZIRLIK ÇARKI] room.prepWheelEnabled ise draft
    // gerçekte LOBBY'den değil, PREP_WHEEL fazı bittikten sonra (bkz. checkPrepWheelComplete/
    // autoResolvePrepWheel) buraya çağrılır — o yüzden PREP_WHEEL de geçerli bir önceki durum.
    if (room.status !== STATUS.LOBBY && room.status !== STATUS.PREP_WHEEL) return { error: 'DRAFT_ALREADY_STARTED' };
    if (!this.roomManager.allConnected(room)) return { error: 'BOTH_PLAYERS_REQUIRED' };
    ...
    if (room.draftMode === 'wheel') {
      ...
      // [KULLANICI ŞİKAYETİ, KARARLAŞTIRILDI — PITY SİSTEMİ] clientId başına "art arda kaç kez
      // kötü havuzdan segment geldi" sayacı (bkz. resolveSpin) — draft boyunca kalıcı, kişiye özel.
  ...
  nextRound(room) {
    ...
    // bu oyuncunun katıldığı İLK pozisyon-turunda (hangi pozisyon olursa olsun), rekabete hiç
    // girmeden doğrudan bu pozisyonun EN İYİ adayını ÜCRETSİZ (0₺) kazanır, bir kereye mahsus.
    // Kaskad/cascade internals'ına dokunmadan (canlı VE kör mod için AYNI şekilde çalışır) —
    // gerçek bir açık arttırma/kör teklif turu hiç AÇILMADAN önce burada devreye giriyor.
    const jokerHolder = needyPlayers.find((p) => p.prepPerk && p.prepPerk.kind === 'joker' && p.prepPerk.active);
    if (jokerHolder) {
      const main = pickSingle(type, room.draft.takenIds, room.playerPool);
      if (main) {
        room.draft.takenIds.add(main.id);
        jokerHolder.prepPerk.active = false;
        this.assignPlayer(room, jokerHolder, main, type, 0, 'joker');
        room.draft.round = { slotType: type, kind: 'one_sided', main, resolvedAt: Date.now() };
        this.emitDraft(room, { event: { type: 'joker_used', slotType: type, clientId: jokerHolder.clientId, player: main } });
        setTimeout(() => {
          if (room.status !== STATUS.DRAFT) return;
          room.draft.round = null;
          this.nextRound(room); // bu pozisyonda başka ihtiyacı olan varsa (K>1 idiyse) normal akışa devam
        }, ROUND_RESULT_DELAY_MS);
        return;
      }
      // main bulunamazsa (havuz tükendi, pratikte olası değil) joker'i tüketmeden normal akışa düş.
    }
  ...
  resolveSpin(room, round) {
    // [KULLANICI ŞİKAYETİ, KARARLAŞTIRILDI — PITY SİSTEMİ] Bu turun sahibi art arda kaç kez
    // 'kötü' havuzdan segment aldıysa, bu spin'de 'iyi' havuz segmentlerinin ağırlığı o kadar
    // katlanır (bkz. gameConfig.js WHEEL_PITY_* ve pool.js spinWheelSegment). Streak MAX_STREAK'te
    // tavanlanır — etkisi sınırsız büyümez, sonuç yine de garanti değildir.
    if (!room.draft.wheelPity) room.draft.wheelPity = {};
    const pityStreak = Math.min(room.draft.wheelPity[round.clientId] || 0, WHEEL_PITY_MAX_STREAK);
    const pityBoost = 1 + pityStreak * WHEEL_PITY_BOOST_PER_STREAK;
    const raw = spinWheelSegment(room.draft.wheelSegments, pityBoost);
    // Bu spin'in gerçekte hangi havuzdan geldiğine göre bir SONRAKİ spin için sayacı güncelle
    // (fallback'ler segment'i değiştirebilir ama raw.pool her zaman ÇARKTA GERÇEKTEN dönülen
    // dilimin havuzunu yansıtır — sayaç bunun üzerinden ilerler, gösterilen sonuçtan değil).
    if (raw.pool === 'kötü') room.draft.wheelPity[round.clientId] = (room.draft.wheelPity[round.clientId] || 0) + 1;
    else if (raw.pool === 'iyi') room.draft.wheelPity[round.clientId] = 0;
  ...
  submitBid(room, clientId, amount) {
    ...
    // [KULLANICI İSTEĞİ, KARARLAŞTIRILDI — HAZIRLIK ÇARKI] "🛡️ Anti-Snipe Kalkanı" perk'i —
    // draft boyunca SÜREKLİ aktif, bu oyuncunun tekliflerinin anti-snipe uzatması tetiklemesini
    // engeller (ama süre yine de normal şekilde işliyor, sadece BU teklif uzatma yapmıyor).
    const hasAntiSnipeShield = bidder.prepPerk && bidder.prepPerk.kind === 'anti_snipe_shield' && bidder.prepPerk.active;
    const msLeft = round.deadline - Date.now();
    if (msLeft < ANTI_SNIPE_WINDOW_MS && !hasAntiSnipeShield) {
