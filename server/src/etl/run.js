...
async function main() {
  ...
  // yükseltmek için eklenen takma adlar (%91.0→%91.6) — her biri UYGULANMADAN ÖNCE
  // `verifyAliasGuesses.js` ile players.json havuzunda GERÇEKTEN TEK bir adaya çıktığı doğrulandı
  // (La Liga'daki disiplinle aynı). Bazı tahminler (ör. "João Cancelo", "Kim Min Jae",
  // "Al-Musrati") havuzda HİÇ bulunamadığı için (muhtemelen ETL'in "aktif oyuncu" filtresine
  // takılmışlar ya da veri setinde hiç yoklar, ya da — Al-Musrati örneğinde — iki dosya birbiriyle
  // ÇELİŞEN kulüp bilgisi veriyor, hangisinin güncel olduğu belirsiz) BİLEREK eklenmedi — yanlış
  // eşleşme riskini göze almaktansa unmatched bırakmak tercih edildi. Bu tablo AYNI ZAMANDA
  // `reapplyFc26Overrides.js`'te de var (raw ETL verisi olmadan players.json'ı yeniden üretebilmek
  // için bağımsız bir kopya) — ikisi senkron tutulmalı.
  const PREMIER_LEAGUE_NAME_ALIASES = {
    'zubimendi': 'martin zubimendi', 'benjamin white': 'ben white', 'kepa': 'kepa arrizabalaga',
    'mosquera': 'cristhian mosquera', 'palhinha': 'joao palhinha', 'amad': 'amad diallo', 'igor': 'igor julio',
  };
  const BUNDESLIGA_NAME_ALIASES = { 'grimaldo': 'alejandro grimaldo' };
  const SERIE_A_NAME_ALIASES = {
    'de gea': 'david de gea', 'morata': 'alvaro morata', 'andre franck zambo anguissa': 'frank anguissa',
  };
